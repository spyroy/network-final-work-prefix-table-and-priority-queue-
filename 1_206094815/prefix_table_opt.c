...
{ 
	...
	for(i = 0; i< mask; i++){
		...
		if(i == mask-1){
			if(key[i] == 1){
				if(temp->one == NULL && temp->zero == NULL){
				...
				else if(temp->one == NULL && temp->zero != NULL && temp->zero->action == act){
					free(temp->zero);
					temp->zero = NULL;
					temp->action = act;
					total_nodes--;	
				...
				// in case the united node is not a leaf
				else if(temp->one == NULL && temp->zero != NULL && temp->zero->action == act && (temp->zero->zero != NULL || temp->zero->one != NULL)){
					temp->action = act;
					temp->zero->action = ' ';	
				}
				else{
			...
			if(key[i] == 0){
				...
				else if(temp->zero == NULL && temp->one != NULL && temp->one->action == act){
					free(temp->one);
					temp->one = NULL;
					temp->action = act;
					total_nodes--;	
				...
				// in case the united node is not a leaf
				else if(temp->zero == NULL && temp->one != NULL && temp->one->action == act && (temp->one->zero != NULL || temp->one->one != NULL)){
					temp->action = act;
					temp->zero->action = ' ';	
				}
				else{
...
struct node* delete(struct node** node,int* key,int* tmp,int index,int mask,char act){
	...
	if(((*node) != NULL && (*node)->action == act && index == mask) || ((*node) != NULL && (*node)->action == act && index == mask-1)){
		printf("removed ");
